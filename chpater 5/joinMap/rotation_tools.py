...
class Isometry:
    def __init__(self, q:Quaternion, t):
        self.q = q
        self.t = t.reshape((3, 1))

    def translation_matrix(self):
        rotation_matrix = self.q.convert_to_rotation_matrix()
        tmp = np.hstack((rotation_matrix, self.t))
        _ = np.array([[0,0,0, 1]])
        res = np.vstack((tmp, _))
        return res

    def __mul__(self, other):
        if len(other.ravel())==3:
            other = other.reshape((3, 1))
            other = np.vstack((other, [[1]]))
        return self.translation_matrix() @ other

    def __str__(self):
        return f'{self.translation_matrix()}'
